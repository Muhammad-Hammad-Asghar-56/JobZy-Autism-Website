...
const EmployerApplications = () => {
  ...
  const handleStatusChange = async (jobId, applicantId, newStatus) => {
    try {
      // Assume JobsApi.updateApplicantStatus exists
      await JobsApi.updateApplicantStatus(jobId, applicantId, newStatus);
      setJobs((prevJobs) =>
        prevJobs.map((job) =>
          job.jobId === jobId
            ? {
                    ...
                    <table className="min-w-full border mt-4 text-left text-sm">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="py-2 px-4 border">Email</th>
                          <th className="py-2 px-4 border">Status</th>
                          <th className="py-2 px-4 border">Change Status</th>
                        </tr>
                      </thead>
                      <tbody>
                        {job.applicants.map((app, index) => (
                          <tr
                            key={app.user?._id || `applicant-${index}`}
                            className="hover:bg-gray-50">
                            <td className="py-2 px-4 border font-medium">
                              {app.user
                                ? `${app.user.title || ""} ${
